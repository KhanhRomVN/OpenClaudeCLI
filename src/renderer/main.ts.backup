...
declare global {
  interface Window {
    claude: {
      ...
      exportConversationMarkdown: (conversationData: {
        title: string;
...
let currentConversationMessages: Array<{
  role: string;
  content: string;
  timestamp?: string;
}> = [];
...
function renderAttachmentList() {
  ...
  const pills = pendingAttachments
    .map((a) => {
      const icon = a.file_type?.startsWith("image/")
        ? imageIconSvg
        : fileIconSvg;
      return `
      <div class="attachment-pill" data-id="${a.id}">
        ...
        <button class="attachment-remove" data-id="${
          a.id
        }" title="Remove">✕</button>
      </div>
    `;
  ...
  containers.forEach(({ list, status }) => {
    ...
    if (status) {
      status.textContent = uploadingAttachments
        ? "Uploading attachments…"
        : attachmentError;
      status.style.display =
        uploadingAttachments || attachmentError ? "block" : "none";
...
function showChat() {
  ...
  if (sidebarTab) sidebarTab.classList.remove("hidden");
  if (modelBadge)
    modelBadge.textContent = modelDisplayNames[selectedModel] || "Opus 4.5";
}
...
function renderConversationItem(c: Conversation): string {
  return `
    <div class="conv-item ${
      c.uuid === conversationId ? "active" : ""
    }" data-uuid="${c.uuid}" data-starred="${c.is_starred || false}">
      <div class="conv-item-row">
        <div class="conv-item-info" data-action="load" data-uuid="${c.uuid}">
          <div class="conv-item-title">${escapeHtml(
            c.name || c.summary || "New conversation"
          )}</div>
          <div class="conv-item-date">${formatDate(c.updated_at)}</div>
        </div>
        <button class="conv-menu-btn" data-action="menu" data-uuid="${
          c.uuid
        }">⋯</button>
      </div>
      <div class="conv-dropdown" id="conv-dropdown-${c.uuid}">
        <div class="conv-dropdown-item" data-action="star" data-uuid="${
          c.uuid
        }" data-starred="${!c.is_starred}">
          <span class="conv-dropdown-icon">${c.is_starred ? "☆" : "★"}</span>
          <span>${c.is_starred ? "Unstar" : "Star"}</span>
        </div>
        <div class="conv-dropdown-item" data-action="rename" data-uuid="${
          c.uuid
        }">
          <span class="conv-dropdown-icon">✎</span>
          <span>Rename</span>
        </div>
        <div class="conv-dropdown-item delete" data-action="delete" data-uuid="${
          c.uuid
        }">
          <span class="conv-dropdown-icon">✕</span>
          <span>Delete</span>
...
function startRenameConversation(uuid: string) {
  ...
  titleEl.innerHTML = `<input type="text" class="conv-rename-input" value="${escapeHtml(
    currentName
  )}" data-uuid="${uuid}">`;
...
const FALLBACK_FAVICON =
  "data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAxNiAxNiI+PGNpcmNsZSBjeD0iOCIgY3k9IjgiIHI9IjciIGZpbGw9IiNkZGQiLz48L3N2Zz4=";
...
function addMessage(
  ...
  content: string,
  raw = false,
  storedParentUuid: string | null = null,
  extraClasses = "",
      ...
      .map((a) => {
        ...
        <div class="message-attachment-row">
          ...
          <div class="message-attachment-info">
            <div class="message-attachment-name">${escapeHtml(
              a.file_name
            )}</div>
            ${
              a.file_size
                ? `<div class="message-attachment-size">${formatFileSize(
                    a.file_size
                  )}</div>`
                : ""
            }
  ...
  if (role === "user") {
    el.dataset.parentUuid =
      storedParentUuid || parentMessageUuid || conversationId || "";
...
function startEditMessage(msgEl: HTMLElement) {
  ...
  const originalText =
    msgEl.dataset.originalText || contentEl.textContent || "";

  contentEl.innerHTML = `
    <div class="message-edit-container">
      <textarea class="message-edit-textarea">${escapeHtml(
        originalText
      )}</textarea>
...
async function submitEditMessage(msgEl: HTMLElement, newText: string) {
  ...
  currentStreamingElement = addMessage(
    "assistant",
  ...
  try {
    ...
    if (currentStreamingElement) {
      ...
      if (content)
        content.innerHTML =
          '<span style="color:#FF453A">Error: ' + e.message + "</span>";
    }
...
function buildToolResultContent(
  ...
  if (result.type === "rich_link" && result.link) {
    ...
    if (!icon && url) {
      try {
        ...
        icon = `https://www.google.com/s2/favicons?sz=32&domain=${encodeURIComponent(
          domain
        )}`;
      } catch {}
    }
    ...
    return `
      <a class="link-card" href="${escapeHtml(url)}" target="_blank">
        <img class="link-card-icon" src="${escapeHtml(
          icon
        )}" onerror="this.onerror=null;this.src='${FALLBACK_FAVICON}'">
  ...
  if (result.type === "json_block") {
    ...
    if (stdout || stderr || returncode !== undefined) {
      ...
      return hasError
        ? '<div class="file-op error"><span class="file-op-icon">✗</span><span class="file-op-text">Command failed</span></div>'
        : "";
    }
    ...
    if (code && filename) {
      ...
      return `
        <div class="file-preview">
          ...
          <div class="tool-output">${escapeHtml(preview)}${
        code.length > 200 ? "..." : ""
      }</div>
        </div>
      `;
    ...
    if (code) {
      return `<div class="tool-output">${escapeHtml(code.substring(0, 300))}${
        code.length > 300 ? "..." : ""
      }</div>`;
    }
  }
  ...
  if (result.type === "text") {
    ...
    if (text.toLowerCase().includes("success")) {
      return `<div class="file-op success"><span class="file-op-icon">✓</span><span class="file-op-text">${escapeHtml(
        text
      )}</span></div>`;
    }
    return `<div class="tool-output ${isError ? "error" : ""}">${escapeHtml(
      text
    )}</div>`;
  }
  ...
  if (Array.isArray(result)) {
    ...
    for (const item of result.slice(0, 5)) {
      ...
      html += `
        <a class="search-result-item" href="${escapeHtml(
          item.url
        )}" target="_blank">
          <img class="search-result-favicon" src="${escapeHtml(
            favicon
          )}" onerror="this.onerror=null;this.src='${FALLBACK_FAVICON}'">
  ...
  if (result.rich_content) {
    let html = '<div class="chat-links">';
    const items = Array.isArray(result.rich_content)
      ? result.rich_content
      : [result.rich_content];
...
function buildStepItem(step: Step, isActive: boolean): string {
  if (step.type === "thinking") {
          ...
          <div class="step-header">
            ...
            ${
              isActive
                ...
                : `<span class="step-chevron">${chevronSvg}</span>`
            }
          </div>
          <div class="step-content">
            <div class="step-text">${escapeHtml(
              step.thinkingText || step.text || ""
            )}</div>
          </div>
        </div>
      </div>
    ...
    const label =
      message || toolLabels[step.toolName || ""] || `Using ${step.toolName}`;
    const resultHtml = buildToolResultContent(
      step.toolName || "",
      step.toolResult,
      step.isError || false
    );
    ...
    return `
      <div class="step-item tool ${
        step.toolResult ? "" : "active"
      }" data-index="${idx}">
        <div class="step-timeline-col">
          <div class="step-dot-row">
            <div class="step-line-top"></div>
...
function parseStoredMessageContent(content: ContentBlock[]): Step[] {
  ...
  for (const block of content) {
    if (block.type === "thinking") {
      const lastSummary =
        block.summaries && block.summaries.length > 0
      ...
      if (currentToolUse && currentToolUse.toolName === block.name) {
        ...
        if (block.display_content) {
          ...
          if (block.name === "web_search") {
            resultData = (block.content as any[])
            ...
            const textContent = (block.content as any[]).find(
              (c) => c.type === "text"
            );
            if (textContent) {
    ...
    if (conv.chat_messages && conv.chat_messages.length > 0) {
      ...
      if (messagesEl) {
        messagesEl.innerHTML =
          '<div class="empty-state" id="empty-state"><div class="empty-state-icon">✦</div><p>What can I help with?</p><span class="hint">Claude is ready</span></div>';
      }
...
async function login() {
  ...
  try {
    ...
    if (result.success) {
      // Reload accounts after successful login
      await loadAccounts();
...
async function sendFromHome() {
  ...
  try {
    ...
    window.claude
      .generateTitle(conversationId, msg)
      .then(() => {
        loadConversationsList();
      })
      .catch((err) => {
  ...
  if (currentStreamingElement) {
    ...
    if (hasLoadingDots && !hasContent) {
      ...
      if (content)
        content.innerHTML =
          finalHtml ||
          '<span style="opacity:0.5;font-style:italic">Stopped</span>';
    }
  }
...
async function init() {
  ...
  if (isAuth) {
    showHome();
    // Load accounts for account switcher
    await loadAccounts();
    loadConversationsList();
  } else {
    showLogin();
  ...
  window.claude.onMessageThinkingStream((d) => {
    if (currentStreamingElement && d.conversationId === conversationId) {
      const block = streamingBlocks.thinkingBlocks.get(d.blockIndex) || {
        isActive: true,
      };
...
async function loadAccounts() {
  try {
    ...
    const activeResult = await window.claude.getActiveAccount();
    if (activeResult.success && activeResult.account) {
      activeAccountId = activeResult.account.id;
    }
...
function getAccountInitials(account: AccountInfo): string {
  if (account.email) {
  ...
  return account.orgId.charAt(0).toUpperCase();
}

function getAccountLabel(account: AccountInfo): string {
  if (account.email) {
  ...
  return account.orgId.substring(0, 8);
}
...
function renderAccountSwitcher() {
  ...
  const homeAccountAvatar = $("home-account-avatar");
  const homeAccountLabel = $("home-account-label");
  const homeAccountList = $("home-account-list");

  const activeAccount = accounts.find((a) => a.id === activeAccountId);

  // Update both chat and home view buttons
  const updateButton = (
    ...
    if (activeAccount) {
      avatar.textContent = getAccountInitials(activeAccount);
      label.textContent = getAccountLabel(activeAccount);
      btn.classList.add("active");
    } else {
  ...
  updateButton(accountBtn, accountAvatar, accountLabel);
  updateButton(homeAccountBtn, homeAccountAvatar, homeAccountLabel);

  // Render account list HTML
  const accountListHTML =
    accounts.length === 0
      ? '<div class="conv-loading">No accounts</div>'
      : accounts
          .map((account) => {
          ...
          <div class="account-item ${
            ...
            <div class="account-item-avatar">${getAccountInitials(
              account
            )}</div>
            <div class="account-item-info">
              <div class="account-item-email">${escapeHtml(
                account.email || account.name || "Account"
              )}</div>
              <div class="account-item-id">${escapeHtml(
                account.orgId.substring(0, 16)
              )}...</div>
            </div>
            ${
              !isActive
                ? `<button class="account-item-remove" data-account-id="${account.id}" title="Remove account">✕</button>`
                : ""
            }
          </div>
        `;
  ...
  if (accountList) {
    ...
    addAccountListListeners(accountList);
  }

  if (homeAccountList) {
    homeAccountList.innerHTML = accountListHTML;
    addAccountListListeners(homeAccountList);
  }
}

function addAccountListListeners(listElement: HTMLElement) {
  // Add event listeners for account items
  listElement.querySelectorAll(".account-item").forEach((item) => {
    item.addEventListener("click", (e) => {
      const target = e.target as HTMLElement;
      if (target.classList.contains("account-item-remove")) {
        return; // Let the remove button handle it
      }
      const accountId = (item as HTMLElement).dataset.accountId;
      if (accountId) switchToAccount(accountId);
    });
  });

  listElement.querySelectorAll(".account-item-remove").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      const accountId = (btn as HTMLElement).dataset.accountId;
      if (accountId) removeAccountConfirm(accountId);
    });
  });
}
...
function toggleAccountDropdown() {
  ...
  dropdown.classList.toggle("open", isAccountDropdownOpen);

  // Close home dropdown if open
  const homeDropdown = $("home-account-dropdown");
  if (homeDropdown) homeDropdown.classList.remove("open");
}

function toggleHomeAccountDropdown() {
  const dropdown = $("home-account-dropdown");
  if (!dropdown) return;
  ...
  const chatDropdown = $("account-dropdown");
  if (chatDropdown) chatDropdown.classList.remove("open");
}
...
async function switchToAccount(accountId: string) {
  ...
  try {
    const result = await window.claude.switchAccount(accountId);
    if (result.success) {
...
async function removeAccountConfirm(accountId: string) {
  const account = accounts.find((a) => a.id === accountId);
  if (!account) return;

  const label = account.email || account.name || account.orgId.substring(0, 16);
  if (!confirm(`Remove account "${label}"?`)) {
    return;
  }

  try {
    const result = await window.claude.removeAccount(accountId);
    if (result.success) {
      await loadAccounts();
    }
  } catch (e) {
...
function setupEventListeners() {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (
      ...
      !(e.target as HTMLElement).closest(".account-switcher")
    ) {
      const accountDropdown = $("account-dropdown");
      const homeAccountDropdown = $("home-account-dropdown");
      if (accountDropdown) accountDropdown.classList.remove("open");
      if (homeAccountDropdown) homeAccountDropdown.classList.remove("open");
      isAccountDropdownOpen = false;
    }
  });

  // Account switcher (chat view)
  $("account-btn")?.addEventListener("click", (e) => {
    e.stopPropagation();
    toggleAccountDropdown();
  });

  $("add-account-btn")?.addEventListener("click", addNewAccount);

  // Account switcher (home view)
  $("home-account-btn")?.addEventListener("click", (e) => {
    e.stopPropagation();
    toggleHomeAccountDropdown();
  });

  $("home-add-account-btn")?.addEventListener("click", addNewAccount);

...
ipcMain.handle("login", async () => {
  ...
  // This is important when adding a new account while already logged in
  try {
    ...
    console.log("[Auth] Cleared existing session cookies for fresh login");
  } catch (e) {
    console.log("[Auth] No existing cookies to clear");
  }
  ...
  const checkCookies = async (): Promise<{
    ...
    if (sessionKey && orgId) {
      ...
      // Save account info for multi-account support
      try {
        await saveAccount(orgId);
        console.log("[Auth] Account saved successfully");
      } catch (error) {
        console.error("[Auth] Failed to save account:", error);
      }

      // Keep legacy orgId for backward compatibility
...
ipcMain.handle("get-accounts", async () => {
  try {
    ...
    console.error("[Account] Failed to get accounts:", error);
    return { success: false, error: String(error) };
  }
});

ipcMain.handle("switch-account", async (_event, accountId: string) => {
  try {
    const success = await switchAccount(accountId);
    if (success) {
      // Reload the main window to reflect the new account
...
ipcMain.handle("remove-account", async (_event, accountId: string) => {
  try {
    ...
    console.error("[Account] Failed to remove account:", error);
    return { success: false, error: String(error) };
  }
});
...
ipcMain.handle("get-active-account", async () => {
  try {
    ...
    console.error("[Account] Failed to get active account:", error);
    return { success: false, error: String(error) };
  }
});
...
ipcMain.handle("create-conversation", async (_event, model?: string) => {
  ...
  if (result.status !== 200 && result.status !== 201) {
    throw new Error(
      `Failed to create conversation: ${result.status} - ${JSON.stringify(
        result.data
      )}`
    );
  }
...
ipcMain.handle(
  "upload-attachments",
  async (_event, files: UploadFilePayload[]) => {
...
ipcMain.handle(
  ...
  async (
    ...
    if (attachments?.length) {
      ...
      console.log(
        "[API] File IDs:",
        attachments.map((a) => a.document_id).join(", ")
      );
    }
    ...
    const fileIds =
      attachments?.map((a) => a.document_id).filter(Boolean) || [];

...
contextBridge.exposeInMainWorld("claude", {
  ...
  createConversation: (model?: string) =>
    ipcRenderer.invoke("create-conversation", model),
  getConversations: () => ipcRenderer.invoke("get-conversations"),
  loadConversation: (convId: string) =>
    ipcRenderer.invoke("load-conversation", convId),
  deleteConversation: (convId: string) =>
    ipcRenderer.invoke("delete-conversation", convId),
  renameConversation: (convId: string, name: string) =>
    ipcRenderer.invoke("rename-conversation", convId, name),
  starConversation: (convId: string, isStarred: boolean) =>
    ipcRenderer.invoke("star-conversation", convId, isStarred),
  ...
  stopResponse: (conversationId: string) =>
    ipcRenderer.invoke("stop-response", conversationId),
  ...
  // Spotlight functions
  spotlightResize: (height: number) =>
    ipcRenderer.invoke("spotlight-resize", height),
  spotlightSend: (message: string) =>
    ipcRenderer.invoke("spotlight-send", message),
  onSpotlightStream: (
    callback: (data: { text: string; fullText: string }) => void
  ) => {
  ...
  onSpotlightThinking: (
    callback: (data: { isThinking: boolean; thinkingText?: string }) => void
  ) => {
    ipcRenderer.on("spotlight-thinking", (_event, data) => callback(data));
  },
  onSpotlightThinkingStream: (
    callback: (data: { thinking: string }) => void
  ) => {
    ipcRenderer.on("spotlight-thinking-stream", (_event, data) =>
      callback(data)
    );
  },
  onSpotlightTool: (

...
export async function saveAccount(
  ...
  const accountInfo: import("../types").AccountInfo = {
    id: orgId,
    orgId,
    sessionKey,
    email,
  ...
  store.set("activeAccountId", orgId);
...
export async function switchAccount(accountId: string): Promise<boolean> {
  ...
  try {
    ...
    console.log(
      `[Account] Switched to account: ${accountId} (${
...
export async function removeAccount(accountId: string): Promise<boolean> {
  ...
  if (activeId === accountId) {
    if (filteredAccounts.length > 0) {
      ...
      const mostRecent = filteredAccounts.sort(
      ...
      store.set("activeAccountId", undefined);
    }
  }
...
function normalizeAttachmentResponse(
  data: any,
  fallback: UploadFilePayload
): AttachmentPayload {
  const doc = data?.document || data || {};
  const documentId =
    doc.document_id || doc.file_uuid || doc.uuid || doc.id || doc.file_id;
  const fileUrl =
    doc.file_url ||
    doc.preview_url ||
    doc.thumbnail_url ||
  ...
  return {
    ...
    extracted_content:
      doc.extracted_content || doc.extract || doc.extracted_text,
  };
}
...
export async function prepareAttachmentPayload(
  ...
  const boundary =
    "----ElectronFormBoundary" + crypto.randomBytes(16).toString("hex");
    ...
    request.on("response", (response) => {
      if (response.statusCode !== 200) {
        ...
        response.on("end", () => {
          reject(
            new Error(
              `Completion failed: ${response.statusCode} - ${errorData}`
            )
          );
        });
        return;
